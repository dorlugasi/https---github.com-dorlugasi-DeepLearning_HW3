...
class DecoderCNN(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # ====== YOUR CODE: ======
        modules += [
            nn.Conv2d(in_channels, 512, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(512),
...
class VAE(nn.Module):
    ...
    def encode(self, x):
        ...
        z = mu + eps * torch.exp(0.5 * log_sigma2)
    ...
    def decode(self, z):
        ...
        # ====== YOUR CODE: ======
        h = self.fc_z_to_h(z)
        h = h.view(z.shape[0], *self.features_shape)
    ...
    def sample(self, n):
        ...
        with torch.no_grad():
            ...
            #    the mean, i.e. psi(z).
            # ====== YOUR CODE: ======
            z_dim = self.fc_z_to_h.in_features
...
def vae_loss(x, xr, z_mu, z_log_sigma2, x_sigma2):
    ...
    dx = x[0].numel()  # C*H*W
    diff = (x - xr).view(N, -1)
    data_loss = (diff.pow(2).sum(dim=1) / (x_sigma2 * dx)).mean()
    sigma2 = torch.exp(z_log_sigma2)
    kld_per_sample = (sigma2 + z_mu.pow(2) - 1.0 - z_log_sigma2).sum(dim=1)
    kldiv_loss = kld_per_sample.mean()
