...
class Trainer(abc.ABC):
    ...
    def fit(
        ...
        for epoch in range(num_epochs):
            ...
            train_loss.append(float(torch.mean(torch.tensor(train_result.losses))))
            train_acc.append(float(torch.mean(torch.tensor(train_result.accuracy))))

            test_result  = self.test_epoch(dl_test,  verbose=verbose, **kw)
            test_loss.append(float(torch.mean(torch.tensor(test_result.losses))))
            test_acc.append(float(torch.mean(torch.tensor(test_result.accuracy))))
...
class RNNTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        loss = self.loss_fn(y_pred.reshape(-1, y_pred.shape[-1]), y.reshape(-1))
        loss.backward()
        self.optimizer.step()
    
        preds = y_pred.argmax(dim=-1)
        num_correct = (preds == y).sum() / x.shape[0]        
        # ========================
